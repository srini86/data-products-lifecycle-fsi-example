...
-- 2. Regenerate dbt model from updated contract (via CoCo $data-product-generator)
...
3. Regenerate dbt model via CoCo $data-product-generator
4. Deploy the updated model (adds new columns to existing table)
5. Update monitoring for new columns

NOTE (for CoCo-generated evolution SQL): Use one ALTER TABLE ... ADD COLUMN
statement per column — Snowflake does not support comma-separated
ADD COLUMN IF NOT EXISTS in a single ALTER TABLE.
*/
...
-- ============================================================================
/*
Use CoCo to regenerate only the affected artifacts from the updated contract:

$data-product-generator
#05_refine/_example/churn_risk_data_contract_v2.yaml
The contract has been updated to v2 with 6 new columns:
products_held_90d_ago, product_downgrade_flag, estimated_clv,
clv_tier, action_confidence_score, vulnerability_indicator.
Regenerate the dbt model SQL and schema.yml for the new columns only.
Leave unchanged artifacts in place.
Save to 03_deliver/dbt_project/models/retail_customer_churn_risk.sql
*/
...
-- ============================================================================
/*
Deploy the updated model using dbt:

  - Run: dbt run --select retail_customer_churn_risk
  - The model creates/replaces the table with all columns including new ones
...
INSERT INTO DATA_PRODUCT_LINEAGE (
    ...
    PARSE_JSON('{
        ...
        "generated_by": "CoCo data-product-generator",
...
3. ✅ Generated v2.0 model from contract (via CoCo $data-product-generator)
...
and generated by CoCo. This script handles deployment only.

To evolve again (v2.0 → v3.0):
1. Archive current version as snapshot
2. Update contract YAML with new requirements  
3. Regenerate code via CoCo $data-product-generator
